...
3. Create the n8n variable `ONBOARDING_INTAKE_TOKEN` (requires Variables support).
4. Attach a Gmail credential to `Send Controlled Reconcile Alert` and replace `ops@example.com`.
5. Safe test: send the token as the `x-onboarding-token` header (`Bearer` also accepted). POST with a wrong token → expect `401`; POST a valid body with a `deal_id`, verified email, service code, and ISO kickoff slot → inspect the claim and intent rows; POST the identical body again → confirm no new rows. The payload accepts a `child_fixtures` object (honored only when `smoke_tag` starts with `TEST-FIX` or `TEST-DRAFT`) to simulate step results without any real step workflows.
