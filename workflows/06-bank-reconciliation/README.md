...
- `Emit`/`Insert` node pairs write six row types to n8n Data Tables: bank transactions, invoices, allocations, review queue, paid signals, and a run summary. A Gmail node sends the operator a summary; the webhook responds with matched/held/signal counts.
...
flowchart TD
  ...
  E --> SUM(Summary email and response)
```
...
2. Create six Data Tables — `Recon_Bank_Transactions`, `Recon_Invoices`, `Recon_Allocations`, `Recon_Review_Queue`, `Recon_Paid_Signals`, `Recon_Run_Summaries` — with columns matching the Insert nodes, then re-point every `Insert`/`Get` Data Table node at your table IDs (the IDs in the export belong to the source instance).
