...
- **Recovery sweep** (`KSeF Manual Sweep` with `recovery`, plus the 30-minute schedule) — picks up unresolved states and queries submission status by `client_submission_id` first. The adapter returns `ACCEPTED`, `NOT_FOUND`, `UNKNOWN`, or `ERROR`; a missing local mock record is `UNKNOWN`, not evidence of absence. Recovery adopts an actual reference and UPO from `ACCEPTED`, retries only after an authoritative `NOT_FOUND`, and otherwise appends hold evidence with zero submit calls. Sends a throttled ops alert (Gmail) when unresolved items age past the window.

The workflow has exactly one manual trigger. `Route Manual Sweep` maps `submission` to the submission entry, `recovery` to the recovery entry, and `intake_fixture` to the fixture intake entry. It emits each manual execution to one branch only, so the three manual paths do not fan out or race each other inside one execution.
...
flowchart TD
  ...
  M[KSeF Manual Sweep] --> S{KSEF_MANUAL_SWEEP_MODE}
  S -- "submission / unset / unknown" --> G{Preconditions pass?}
  S -- intake_fixture --> F[Build synthetic intake fixture]
  F --> P
  S -- recovery --> RC[Recovery queries status first]
...
- **One manual selector, one branch.** `KSeF Manual Sweep` always passes through the three-output `Route Manual Sweep`. The optional `KSEF_MANUAL_SWEEP_MODE` variable accepts `submission`, `recovery`, or `intake_fixture`; missing and unrecognized values default to `submission`. The router selects exactly one output, preventing the manual fixture, submission, and recovery branches from starting together.
...
4. Optionally create `KSEF_MANUAL_SWEEP_MODE` with one of `submission`, `recovery`, or `intake_fixture`. If it is absent or unrecognized, the manual selector defaults to `submission`.
5. Attach your Gmail credential to the two send nodes and replace `ops@example.com` with your ops inbox. The template ships without credentials.
6. Safe test: send the token as the `x-ksef-token` header (a `Bearer` Authorization header also works). `curl -X POST .../webhook/ksef-intake` with a wrong token and expect a `401` fail-closed body; then send an explicit invoice object containing seller and buyer NIPs, invoice number, issue date, gross amount, currency, category, `<Faktura...` XML, whitelist status, approval record, auth adapter, and mock submit mode. Check for a `RECEIVED` lifecycle row whose snapshot matches the request and has no submission. An empty invoice or an invoice missing either adapter field must become `VALIDATION_REJECTED`, never a populated example. To exercise the manual paths, set `KSEF_MANUAL_SWEEP_MODE` to one value at a time and run `KSeF Manual Sweep`; inspect the four tables after each run. The built-in submission and recovery scenarios cover happy path, timeout, crash-after-intent, 429, bad NIP, and injection attempts against a local mock.
...
- The manual selector does not fan out: one trigger execution enters exactly one selected branch. Duplicate protection still uses Data Table read-backs plus workflow static data, not an atomic lock, so separate concurrent executions (including a scheduled recovery overlapping a manual run) are not serialized. Run one sweep at a time.
